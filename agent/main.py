...
Available models: sonnet, opus, haiku (or full model IDs)
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Max iterations for agentic mode (default: 500)"
    )
    parser.add_argument(
        "--dry-run", "-n",
        action="store_true",
...
def main():
    ...
    if args.agentic:
        ...
        # Save JSON report
        json_path = output_dir / f"agentic_report_{timestamp}.json"
        json_path.write_text(json.dumps(report_data, indent=2))
        print(f"\nJSON report: {json_path}")

        # Generate markdown report
        md_lines = [
            "# AI Performance Agent Report (Agentic Mode)",
            "",
            f"**Generated**: {datetime.utcnow().isoformat()}",
            f"**SUT**: {args.sut}",
            f"**Benchmark Node**: {args.benchmark}",
            ...
            "## Performance Results",
            "",
            "| Workload | Baseline (rps) | After (rps) | Improvement |",
            "|----------|----------------|-------------|-------------|",
        ]
        ...
        for workload in state.baseline_rps:
            if workload in state.current_rps:
                ...
                if before > 0:
                    pct = ((after - before) / before) * 100
                    sign = "+" if pct >= 0 else ""
                    md_lines.append(f"| {workload} | {before:,.0f} | {after:,.0f} | **{sign}{pct:.1f}%** |")
        ...
        md_lines.extend([
            ...
            "## Agent Summary",
            "",
            state.summary if state.summary else "No summary provided.",
            "",
            "---",
            "",
            "## Token Usage",
        ...
        for u in usage:
            ...
            md_lines.append(
                f"| {u.get('model', 'unknown')} | {u.get('input_tokens', 0):,} | "
                f"{u.get('output_tokens', 0):,} | {total_tok:,} | "
                f"{u.get('api_calls', 0)} | ${u.get('cost_usd', 0):.4f} |"
            )

        md_lines.append(f"| **Total** | **{total_tokens:,}** | - | - | **{total_calls}** | **${total_cost:.2f}** |")
        ...
        md_lines.extend([
            ...
            "## Actions Taken",
            "",
            f"Total actions: {len(runner.get_decision_log())}",
            "",
        ])
        ...
        if state.actions_taken:
            ...
            for action in state.actions_taken:
                if action.get("type") == "write_file":
                    md_lines.append(f"- `{action.get('path', 'unknown')}`")
            md_lines.append("")

        md_lines.extend([
            "---",
            "",
            "*Report generated by AI Performance Agent v1.0 (Agentic Mode)*",
        ])

        md_path = output_dir / f"agentic_report_{timestamp}.md"
        md_path.write_text("\n".join(md_lines))
